...
# conditional rotation modules here
try:
	import cv2
	CV2_AVAILABLE = True
except ImportError:
...
def detect_page_rotation_opencv(img: Image.Image, logger: logging.Logger) -> int:
	"""Detect page rotation using OpenCV layout heuristics; returns 0 or 90 degrees with confidence [0..100]."""
	if not CV2_AVAILABLE:
		logger.debug("[ROTATION][OPENCV] cv2 not available; skipping opencv detection")
		return 0
    
	try:
		rgb = img.convert("RGB")
		arr = np.array(rgb)
		gray = cv2.cvtColor(arr, cv2.COLOR_RGB2GRAY)  # pyright: ignore[reportPossiblyUnboundVariable]
		_, bw = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU) # pyright: ignore[reportPossiblyUnboundVariable]
		kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (25, 3)) # pyright: ignore[reportPossiblyUnboundVariable]
		merged = cv2.morphologyEx(bw, cv2.MORPH_CLOSE, kernel, iterations=1) # pyright: ignore[reportPossiblyUnboundVariable] 
		contours, _ = cv2.findContours(merged, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE) # pyright: ignore[reportPossiblyUnboundVariable]

		img_h, img_w = gray.shape
		min_area = float(img_w * img_h) * 0.00005
		horizontal_score = 0.0
		vertical_score = 0.0
		kept = 0

		for contour in contours:
			x, y, w, h = cv2.boundingRect(contour) # pyright: ignore[reportPossiblyUnboundVariable]
			area = float(w * h)
		...
		if total <= 0:
			logger.debug("[ROTATION][OPENCV] no valid contours for orientation detection")
			return 0

		vertical_ratio = vertical_score / total
		horizontal_ratio = horizontal_score / total
		logger.debug(
			"[ROTATION][OPENCV] contours=%d horizontal_score=%.1f vertical_score=%.1f vertical_ratio=%.4f",
			kept,
			horizontal_score,
			vertical_score,
			vertical_ratio,
		...
		if vertical_ratio > 0.58:
			rotation = 90
			confidence = vertical_ratio * 100.0
		else:
			rotation = 0
			confidence = horizontal_ratio * 100.0

		logger.debug("[ROTATION][OPENCV] Detected rotation=%d° (confidence=%.1f%%)", rotation, confidence)
		return rotation
	except Exception as exc:
		logger.debug("[ROTATION][OPENCV] Detection failed: %s: %s", type(exc).__name__, exc)
		return 0
...
def render_pdf_to_images(pdf_path: Path, dpi: int, tmp_dir: Path, logger: logging.Logger,
                         page_indices: Optional[List[int]] = None, max_input_size: Optional[int] = None) -> tuple[List[Path], List[int]]:
    ...
    for page_index in pages_to_render:
        ...
        detected_rotation = detect_page_rotation_opencv(img, logger)
        rotations.append(detected_rotation)
...
def rotate_to_original(bbox: List[float], detected_rotation: int) -> List[float]:
    """Rotate a normalized bounding box back to the original (unrotated) coordinate space.

    The initial image correction applies ``img.rotate(-detected_rotation)``
    which is a **clockwise** rotation (PIL treats positive angles as CCW).
    To reverse that, this function applies a **counter-clockwise** rotation
    of *detected_rotation* degrees around the centre of the normalised unit
    square ``(0.5, 0.5)``.

    Args:
        bbox: Normalised bounding box ``[x1, y1, x2, y2]`` with values in
              ``[0, 1]`` relative to the corrected image.
        detected_rotation: The rotation angle (in degrees) that was originally
                           detected and corrected.  Common values are 0, 90,
                           180, and 270.

    Returns:
        A new ``[x1, y1, x2, y2]`` list rotated **counter-clockwise** by
        *detected_rotation* degrees so that the coordinates correspond to the
        original, unrotated page.
    """
    angle = detected_rotation % 360
    ...
    if angle == 180:
        # 180° (same CW or CCW): (x, y) → (1 - x, 1 - y)
        return [1.0 - x2, 1.0 - y2, 1.0 - x1, 1.0 - y1]

    if angle == 270:
        # CCW 270° (= CW 90°): (x, y) → (y, 1 - x)
        return [y1, 1.0 - x2, y2, 1.0 - x1]

    # General CCW rotation for arbitrary angles
    import math
    cx, cy = 0.5, 0.5
...
def process_document(
    ...
    with tempfile.TemporaryDirectory() as tmpdir_str:
        ...
        if input_type == "pdf":
            ...
            page_image_paths, rotations = render_pdf_to_images(
        ...
        for page_index in selected_indices:
            ...
            current_rotation = rotations[position]
            ...
            try:
                ...
                for blk in classified_blocks:
                    ...
                    if current_rotation != 0:                    
                        bbox = rotate_to_original(bbox, current_rotation)
